...
exports = module.exports = function(basePath)
{
    ...
    var determineHost = function(configuration, req)
    {
        ...
        if (!host && configuration.virtualHost && configuration.virtualHost.hostHeader)
        {
            host = req.header[configuration.virtualHost.hostHeader];
        }
        ...
        // if the request came through multiple proxies, then it is possible that the host header
        // (coming off of X-Forwarded-Host) may have "," in it because there is a chain of proxies
        // in this case, we want to rewrite back to the last one
        if (host)
        {
            var z = host.indexOf(",");
            if (z > -1)
            {
                // keep the last one
                host = host.split(",").reverse()[0];

                // trim in case there are spaces
                host = util.trim(host);
            }
        }
    ...
    r.virtualDriverConfigInterceptor = function(configuration)
    {
        var loadVirtualDriverGitana = function(req, callback)
        {
            ...
            else
            {
                ...
                {
                    callback({
                        "message": "Cannot dynamically load virtual driver since not virtualDriver configuration block was provided"
                    });
                }
            }
        };
