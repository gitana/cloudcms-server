...
exports = module.exports = function(basePath)
{
    ...
    r.virtualHostInterceptor = function(configuration)
    {
        ...
        return function(req, res, next)
        {
            ...
            {
                console.log("Unable to determine host");
    ...
    r.virtualDriverConfigInterceptor = function(configuration)
    {
        ...
        var loadVirtualDriverConfig = function(req, callback)
        {
            ...
            loadVirtualDriverGitana(req, function(err, gitana) {
                ...
                util.retryGitanaRequest(req, gitana, requestConfig, 2, function(err, response, body) {
                    ...
                    {
                        req.log("Load virtual driver config failed");
                        if (response && response.statusCode)
                        {
                            req.log("Response status code: " + response.statusCode);
                        }
                        if (err) {
                            req.log("Err: " + JSON.stringify(err));
                        }
                        if (body) {
                            req.log("Body: " + body);
                        }
                ...
                fs.exists(gitanaJsonPath, function(exists) {
                    ...
                    if (!exists)
                    {
                        ...
                        loadVirtualDriverConfig(req, function(err, virtualConfig) {
                            ...
                            if (err)
                            {
                                req.log("Unable to load virtual driver config for host: " + req.virtualHost);
                                //req.log("  -> " + err.message);
                                next();
                                return;
                            }
                            ...
                            // store reference to virtual driver configuration
                            req.virtualDriverConfig = configuration.virtualDriver;
