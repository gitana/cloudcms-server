 ...
 * @type {Function}
 */
var retryGitanaRequest = exports.retryGitanaRequest = function(req, gitana, config, maxAttempts, callback)
{
    var _retryHandler = function(gitana, config, currentAttempts, maxAttempts, previousError, cb)
    {
        req.log("Heard invalid_token, attempting retry (" + currentAttempts + " / " + maxAttempts + ")");
        ...
        gitana.getDriver().refreshAuthentication(function(err) {
            ...
            if (err)
            {
                req.log("Failed to refresh access_token: " + JSON.stringify(err));
            }
