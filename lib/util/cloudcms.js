...
exports = module.exports = function(basePath)
{
            ...
            tempStream.on("open", function(fd) {
                ...
                request({
                    ...
                    else
                    {
                        ...
                        callback({ "message": "Unable to load asset from remote store", "code": response.statusCode });
                    }
    ...
    var download = function(host, gitana, repositoryId, branchId, nodeId, attachmentId, nodePath, locale, forceReload, callback)
    {
        ...
        ensureContentDirectory(host, repositoryId, branchId, nodeId, locale, function(err, contentDirectoryPath) {
            ...
            if (nodePath) {
                filePath = path.join(filePath, nodePath);
            }
            if (attachmentId) {
                filePath = path.join(contentDirectoryPath, "attachments", attachmentId + path.extname(nodePath));
            } else {
            ...
            else
            {
                ...
                console.log("GITANA HOST: " + gitanaHost);
     ...
     * @param callback
     */
    var preview = function(host, gitana, repositoryId, branchId, nodeId, nodePath, locale, previewId, size, forceReload, callback)
    {
        // base storage directory
        ensureContentDirectory(host, repositoryId, branchId, nodeId, locale, function(err, contentDirectoryPath) {
            ...
            else
            {
                ...
                saveToDisk(gitanaHost, gitanaPort, uri, gitana, filePath, function(err) {
                    if (err) {
                        console.log("ERR: " + err.message);
                        callback(err);
                    }
    ...
    r.preview = function(host, gitana, repositoryId, branchId, nodeId, nodePath, locale, previewId, size, forceReload, callback)
    {
        preview(host, gitana, repositoryId, branchId, nodeId, nodePath, locale, previewId, size, forceReload, callback);
    };
