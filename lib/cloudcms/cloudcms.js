...
exports = module.exports = function(basePath)
{
    ...
    r.driverInterceptor = function(configuration)
    {
        ...
        {
            if (configuration && configuration.autoRefresh && configuration.autoRefresh.log)
            {
                console.log(text);
            }
        };
        ...
        setInterval(function() {
            ...
            diLog("Driver Interceptor Auto Refresh Starting...");
            ...
            if (process.cache)
            {
                process.cache.each("hostGitanaConfigs", function(host, gitanaConfig) {
                    ...
                    Gitana.connect(gitanaConfig, function(err) {
                        ...
                        diLog(" -> [" + host + "] connected");

                        var g = this;

                        if (err)
                        {
                            diLog(" -> [" + host + "] Caught error while running driverInterceptor auto-refresh");
                            diLog(" -> [" + host + "] " + err);
            ...
            {
                diLog("No cache configured");
            }

            diLog("Driver Interceptor Auto Refresh Finished");
            ...
            readGitanaJson(req, function(err, gitanaConfig) {
                ...
                Gitana.connect(gitanaConfig, function(err) {
                    ...
                    if (err)
                    {
                        ...
                        {
                            // guest mode
                            res.status(err.status).send("Unable to connect to Cloud CMS as guest user");
                            res.send();
                            return;
                        }
                        ...
                        res.status(err.status).send("Cannot connect to Cloud CMS, contact your administrator");
                        res.end();
                        return;
                    }
                    ...
                    if (gitanaConfig)
                    {
                        ...
                        // store into host map so that socket.io can retrieve
                        var host = req.host;
                        if (req.virtualHost) {
                            host = req.virtualHost;
                        }
                        process.cache.write("hostGitanaConfigs", host, gitanaConfig);

                        // store into cache map so that socket.io can use same cache
                        process.cache.write("hostCacheConfigs", host, {
        ...
        return function(req, res, next)
        {
            ...
            if (gitana)
            {
                ...
                if (previewPath || previewNode)
                {
                    ...
                    cloudcmsUtil.preview(host, gitana, repositoryId, branchId, nodeId, nodePath, locale, previewId, size, mimetype, forceReload, function(err, filePath, cacheInfo) {

                        if (err)
                        {
                            //console.log("PREVIEW ERR: " + JSON.stringify(err));
